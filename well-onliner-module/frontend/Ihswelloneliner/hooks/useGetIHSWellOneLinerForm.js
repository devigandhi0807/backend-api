...
import FormDateInputWrapper from 'components/FormDateInputWrapper';
...
const useGetIHSWellOneLinerForm = ({
  ...
  const OperatorCityInput = () => {
    const operatorcityInput = (
      <FormInputWrapper
        ...
        type="text"
        required
        placeholder={messages.operator_cityPlaceHolder}
      />
    );
    return responsive ? (
  ...
  const FormationProducingNameInput = () => {
    const formationproducingnameInput = (
      <FormInputWrapper
        label={messages.formation_producing_nameLabel}
