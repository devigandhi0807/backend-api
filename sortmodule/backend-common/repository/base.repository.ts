...
> extends Repository<T> {
  ...
  async paginate(
    ...
    const whereCondition = await this.pharseFilterQuery(
      searchFilter,
      searchCriteria
    );
    ...
    if (searchFilter.hasOwnProperty('sort_by') && searchFilter.sort_by) {
      if (
        searchFilter.sort_order === 'DESC' ||
        searchFilter.sort_order === 'desc'
      ) {
        findOptions.order = { [searchFilter.sort_by]: 'DESC' };
      } else if (
        searchFilter.sort_order === 'ASC' ||
        searchFilter.sort_order === 'asc'
      ) {
        findOptions.order = {
          [searchFilter.sort_by]: 'ASC'
        };
      } else {
        findOptions.order = {
          [searchFilter.sort_by]: 'DESC'
        };
      }
    } else {
